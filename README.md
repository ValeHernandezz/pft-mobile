# 🎓 Proyecto Final de Tecnicatura | Móvil 📱

Aplicación móvil desarrollada como parte del  **Proyecto Final de Tecnicatura** de la carrera de Licenciatura en Tecnologías de la Información (LTI) de la Universidad Tecnológica del Uruguay (UTEC) diseñada para **usuarios Analistas y Estudiantes**. La app permite la **gestión** de reclamos académicos desde dispositivos móviles. Está desarrollada en **Kotlin**, utilizando **Retrofit2** para consumir **API Rest** del portal web, con **almacenamiento local** de datos de inicio de sesión.

![readme-image-1](https://github.com/user-attachments/assets/f8aab1c6-dd99-4ba1-a668-08bf06703060)

![readme-image-2](https://github.com/user-attachments/assets/58266c50-b6f2-4154-87eb-aaeb248e8854)


## ✔ Funcionalidades

- **Solicitud de Reclamos**: Disponible para estudiantes, quienes pueden enviar reclamos directamente desde sus dispositivos móviles.
- **Modificación de Reclamos**: Los estudiantes tienen la posibilidad de editar sus reclamos existentes.
- **Listado de Reclamos**:
  - *Estudiantes*: Visualización de sus propios reclamos.
  - *Analistas*: Visualización de todos los reclamos realizados por todos los estudiantes.
- **Eliminación de Reclamos**: Permite a los estudiantes eliminar sus reclamos desde la app.
- **Inicio y cierre de sesión**: Los usuarios Analistas y Estudiantes pueden iniciar y cerrar sesión en la aplicación.
- **Persistencia de sesión**: La sesión del usuario se mantiene activa hasta que el mismo decida cerrarla manualmente.


## 🛠️ Construido con

- [![Kotlin](https://img.shields.io/badge/Kotlin-B125EA?style=for-the-badge&logo=kotlin&logoColor=white)](https://kotlinlang.org/) - Lenguaje de programación para desarrollo en Android y aplicaciones Java.
- [![Retrofit2](https://img.shields.io/badge/Retrofit2-49b883?style=for-the-badge&logo=data:image/svg%2bxml;base64,)](https://square.github.io/retrofit/) - Librería para integrar APIs RESTful en aplicaciones Android y Java.
- [![Gradle](https://img.shields.io/badge/gradle-02303A?style=for-the-badge&logo=gradle&logoColor=white)](https://gradle.org/) - Herramienta de automatización de construcción para proyectos Java, Android y más.
- [![Android-Studio](https://img.shields.io/badge/Android_Studio-3DDC84?style=for-the-badge&logo=android-studio&logoColor=white)](https://developer.android.com/studio?hl=es-419) - IDE para el desarrollo de aplicaciones Android.

## 💻 Desarrollado por equipo `Error404`

- [nicomelendez](https://github.com/nicomelendez)
- [ValeHernandezz](https://github.com/ValeHernandezz)
- [martina.fernandez](https://git.utec.edu.uy/martina.fernandez)

## 👩‍💻 Sobre mí
[![portfolio](https://img.shields.io/badge/my_portfolio-000?style=for-the-badge&logo=ko-fi&logoColor=white)](https://valehernandez.vercel.app)
[![linkedin](https://img.shields.io/badge/linkedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/valentina-hernandez-modino)
